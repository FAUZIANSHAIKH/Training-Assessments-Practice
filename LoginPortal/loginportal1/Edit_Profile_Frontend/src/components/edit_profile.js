...
export default class EditProfile extends Component {
    ...
    componentDidMount(){
      axios.get('http://localhost:8018/api/4')
        .then(res => {
          const data = res.data;
   ...
   update(e){
     ...
     //console.log(this.state.user);
      ...
      axios.post('http://localhost:8018/api/updateuser', user)
      .then(res => {
   ...
   pingForOtp(){
    ...
    axios.post('http://localhost:8018/api/sendotp', user);
   }
   ...
   verifyOTP = () => {
    ...
    axios.post('http://localhost:8018/api/verifyotp', {otp:otpValue})
    .then(res => {
      console.log(res);
      if(res.data==="valid"){
          this.update();
