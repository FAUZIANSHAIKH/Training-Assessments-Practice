...
public class MailNotification {
	 ...
	 * 
	 * @Autowired public void MailNotification() {
	 * System.out.println("constructor"); }
	 * 
	 * public void doSendEmailWith(String to) { final String FROM =
	 * "maitreyee.gadwe@publicissapient.com"; final String SUBJECT =
	 * "LoginPortal Update Profile Assistance"; final String HTMLBODY =
	 * "Your profile has been updated !"; final String TEXTBODY =
	 ...
	 * .withMessage(new Message() .withBody(new Body().withHtml(new
	 * Content().withCharset("UTF-8").withData(HTMLBODY)) .withText(new
	 * Content().withCharset("UTF-8").withData(TEXTBODY))) .withSubject(new
	 * Content().withCharset("UTF-8").withData(SUBJECT))) .withSource(FROM);
	 * client.sendEmail(request); System.out.println("Email sent!"); } catch
	 * (Exception ex) { System.out.println("The email was not sent. Error message: "
	 * + ex.getMessage()); }
	 * 
	 * }
	 * 
