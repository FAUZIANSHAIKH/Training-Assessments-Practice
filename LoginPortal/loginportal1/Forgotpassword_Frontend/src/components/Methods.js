...
export default class Methods extends React.Component{
    ...
    click(){
        ...
        Axios.post("http://localhost:8009/forgotpassword/mts",data).then(res => this.setState({data:res.data,checked:data.choice,status:res.data.status}) )
    ...
    render(){
        return(
            <div>
                   ...
                   <input type="radio" name="method" value="1" id="1" onChange={this.click}/>Send link to the email<br/>
                   <input type="radio" name="method" value="2" id="2" onChange={this.click}/>Send OTP to email<br/>
