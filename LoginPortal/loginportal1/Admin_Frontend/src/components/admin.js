...
export default class Admin extends Component {
    ...
    componentDidMount(){
      axios.get('http://localhost:8020/api/getAll')
        .then(res => {
          this.setState({
            data:res.data
          });
  ...
  handleDelete(event,rowData){
    console.log(rowData);
  axios.post("http://localhost:8003/deleteuser/",{userID:rowData.userID})
  window.location.reload();
  }
  handleUndelete(event,rowData){
    console.log(rowData);
  axios.post("http://localhost:8003/undeleteuser/",{userID:rowData.userID})
  window.location.reload();
  }
  handlePurge(event,rowData){
    console.log(rowData);
  axios.post("http://localhost:8003/purgeuser/",{userID:rowData.userID})
  window.location.reload();
  }
